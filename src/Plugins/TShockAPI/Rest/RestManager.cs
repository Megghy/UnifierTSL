...
namespace TShockAPI
{
    ...
    public class RestManager
    {
        ...
        {
            // Server Commands
            if (TShock.Config.GlobalSettings.EnableTokenEndpointAuthentication)
            {
            ...
            //server commands
            Rest.RegisterRedirect("/server/list", "/v2/server/list");
            ...
            Rest.Register(new SecureRestCommand("/v2/server/list", ServerList, RestPermissions.restbroadcast));
        ...
        //	Utils.StopServer(!GetBool(args.Parameters["nosave"], false), reason);

        //	return RestResponse("The server is shutting down");
        //}

        [Description("")]
        [Route("/v3/server/list")]
        [Token]
        private object ServerList(RestRequestArgs args) {
        ...
        private object ServerBroadcast(RestRequestArgs args)
        {
            ...
            if (string.IsNullOrWhiteSpace(serverName)) {
                foreach (var server in UnifiedServerCoordinator.Servers.Where(s => s.IsRunning)) {
                    server.GetExtension<TSServerPlayer>().BCInfoMessage(msg);
                }
                TShock.Log.Info($"Broadcast From Rest: {msg}");
            }
            else {
        ...
        private object ServerMotd(RestRequestArgs args)
        {
            ...
            if (!File.Exists(motdFilePath))
                return RestError(GetString("The motd.txt was not found."), "500");
        ...
        private object ServerRules(RestRequestArgs args)
        {
            ...
            if (!File.Exists(rulesFilePath))
                return RestError(GetString("The rules.txt was not found."), "500");
        ...
        private object ServerStatusV2(RestRequestArgs args) {
            ...
            var ret = new RestObject()
            {
                ...
                {"port", UnifiedServerCoordinator.ListenPort},
                {"playercount", Utils.GetActivePlayerCount(server)},
                {"maxplayers", TShock.Config.GlobalSettings.MaxSlots},
                ...
                {"serverpassword", !string.IsNullOrEmpty(UnifiedServerCoordinator.ServerPassword)}
            };
            ...
            if (GetBool(args.Parameters["rules"], false))
            {
                ...
                rules.Add("AutoSave", TShock.Config.GlobalSettings.AutoSave);
                rules.Add("DisableBuild", settings.DisableBuild);
                rules.Add("DisableClownBombs", settings.DisableClownBombs);
                rules.Add("DisableDungeonGuardian", settings.DisableDungeonGuardian);
                rules.Add("DisableInvisPvP", settings.DisableInvisPvP);
                rules.Add("DisableSnowBalls", settings.DisableSnowBalls);
                rules.Add("DisableTombstones", settings.DisableTombstones);
                rules.Add("EnableWhitelist", TShock.Config.GlobalSettings.EnableWhitelist);
                rules.Add("HardcoreOnly", settings.HardcoreOnly);
                rules.Add("PvPMode", settings.PvPMode);
                rules.Add("SpawnProtection", settings.SpawnProtection);
                rules.Add("SpawnProtectionRadius", settings.SpawnProtectionRadius);
                rules.Add("ServerSideInventory", server.Main.ServerSideCharacter);
        ...
        private object UserCreateV2(RestRequestArgs args)
        {
            ...
            if (string.IsNullOrWhiteSpace(group))
                group = TShock.Config.GlobalSettings.DefaultRegistrationGroupName;
        ...
        private object WorldChangeSaveSettings(RestRequestArgs args)
        {
            ...
            TShock.Config.GlobalSettings.AutoSave = autoSave;
        ...
        private object WorldChangeSaveSettingsV3(RestRequestArgs args)
        {
            ...
            {
                if (TShock.Config.GlobalSettings.AutoSave)
                {
        ...
        private object WorldButcher(RestRequestArgs args) {
            if (!FindTargetServer(args, out var server, out var resp)) {
                return resp;
            }
            bool killFriendly;
            ...
            {
                if (server.Main.npc[i].active && server.Main.npc[i].type != 0 && !server.Main.npc[i].townNPC && (!server.Main.npc[i].friendly || killFriendly))
                {
                    serverPlr.StrikeNPC(i, 99999, 90f, 1);
                    killcount++;
                }
            }
        ...
        private object WorldRead(RestRequestArgs args) {
            ...
            {
                {"name", server.Name},
                {"size", server.Main.maxTilesX + "*" + server.Main.maxTilesY},
                {"time", server.Main.time},
                {"daytime", server.Main.dayTime},
                {"bloodmoon", server.Main.bloodMoon},
                {"invasionsize", server.Main.invasionSize}
            };
        }
        ...
        private object WorldBloodmoon(RestRequestArgs args) {
            ...
            server.Main.bloodMoon = bloodmoon;

            resp = RestResponse(GetString($"Blood Moon has been set to {bloodmoon}"));
        ...
        private object WorldBloodmoonV3(RestRequestArgs args) {
            ...
            {
                return RestResponse(GetString($"Bloodmoon state: {(server.Main.bloodMoon ? "Enabled" : "Disabled")}"));
            }
            server.Main.bloodMoon = bloodmoon;
        ...
        static bool FindTargetServer(RestRequestArgs args, [NotNullWhen(true)] out ServerContext? server, [NotNullWhen(false)] out RestObject? resp) {
            var serverName = args.Parameters["server"];
            if (string.IsNullOrWhiteSpace(serverName)) {
                server = null;
                resp = RestMissingParam("server");
                return false;
            }
            server = UnifiedServerCoordinator.Servers.FirstOrDefault(s => s.IsRunning && s.Name == serverName);
            if (server == null) {
                resp = RestResponse($"Server {serverName} was not found.");
                return false;
            }
            resp = null;
            return true;
