...
namespace TShockAPI
{
    ...
    public partial class TSPlayer
    {
        ...
        public bool IsBeingDisabled() {
            ...
            || !IsLoggedIn && GetCurrentSettings().RequireLogin;
        }
        ...
        public bool HasHackedItemStacks(bool shouldWarnPlayer = false) {
            ...
            return HasHackedStacks(server, template, TPlayer.inventory, "Stack cheat detected. Remove item {0} ({1}) and then rejoin.", true, shouldWarnPlayer)
                | HasHackedStacks(server, template, TPlayer.armor, "Stack cheat detected. Remove armor {0} ({1}) and then rejoin.", true, shouldWarnPlayer)
                | HasHackedStacks(server, template, TPlayer.dye, "Stack cheat detected. Remove dye {0} ({1}) and then rejoin.", true, shouldWarnPlayer)
                | HasHackedStacks(server, template, TPlayer.miscEquips, "Stack cheat detected. Remove item {0} ({1}) and then rejoin.", true, shouldWarnPlayer)
                | HasHackedStacks(server, template, TPlayer.miscDyes, "Stack cheat detected. Remove item dye {0} ({1}) and then rejoin.", true, shouldWarnPlayer)
                | HasHackedStacks(server, template, TPlayer.bank.item, "Stack cheat detected. Remove piggy-bank item {0} ({1}) and then rejoin.", true, shouldWarnPlayer)
                | HasHackedStacks(server, template, TPlayer.bank2.item, "Stack cheat detected. Remove safe item {0} ({1}) and then rejoin.", true, shouldWarnPlayer)
                | HasHackedStack(server, template, TPlayer.trashItem, "Stack cheat detected. Remove trash item {0} ({1}) and then rejoin.", false, shouldWarnPlayer)
                | HasHackedStacks(server, template, TPlayer.bank3.item, "Stack cheat detected. Remove Defender's Forge item {0} ({1}) and then rejoin.", true, shouldWarnPlayer)
                | HasHackedStacks(server, template, TPlayer.bank4.item, "Stack cheat detected. Remove Void Vault item {0} ({1}) and then rejoin.", true, shouldWarnPlayer)
                | HasHackedStacks(server, template, TPlayer.Loadouts[0].Armor, "Stack cheat detected. Remove Loadout 1 item {0} ({1}) and then rejoin.", true, shouldWarnPlayer)
                | HasHackedStacks(server, template, TPlayer.Loadouts[0].Dye, "Stack cheat detected. Remove Loadout 1 item {0} ({1}) and then rejoin.", true, shouldWarnPlayer)
                | HasHackedStacks(server, template, TPlayer.Loadouts[1].Armor, "Stack cheat detected. Remove Loadout 2 item {0} ({1}) and then rejoin.", true, shouldWarnPlayer)
                | HasHackedStacks(server, template, TPlayer.Loadouts[1].Dye, "Stack cheat detected. Remove Loadout 2 item {0} ({1}) and then rejoin.", true, shouldWarnPlayer)
                | HasHackedStacks(server, template, TPlayer.Loadouts[2].Armor, "Stack cheat detected. Remove Loadout 3 item {0} ({1}) and then rejoin.", true, shouldWarnPlayer)
                | HasHackedStacks(server, template, TPlayer.Loadouts[2].Dye, "Stack cheat detected. Remove Loadout 3 item {0} ({1}) and then rejoin.", true, shouldWarnPlayer);
        }

        private bool HasHackedStacks(ServerContext server, Item template, Item[] stacks, string warning, bool checkNegative, bool shouldWarnPlayer) {
            bool detected = false;
            foreach (var stack in stacks) {
                detected |= HasHackedStack(server, template, stack, warning, checkNegative, shouldWarnPlayer);
            }
            return detected;
        }

        private bool HasHackedStack(ServerContext server, Item template, Item stack, string warning, bool checkNegative, bool shouldWarnPlayer) {
            if (stack == null || stack.type == 0) {
                return false;
            }

            template.netDefaults(server, stack.type);
            template.Prefix(server, stack.prefix);
            template.AffixName();
            if (stack.stack <= template.maxStack && (!checkNegative || stack.stack >= 0)) {
                return false;
            }

            if (shouldWarnPlayer) {
                SendErrorMessage(GetString(warning, template.Name, stack.stack));
            }
            return true;
        }
        ...
        public bool IsInRange(int x, int y, int range = 32) {
            ...
            if (setting.RangeChecks && ((rgX > range) || (rgY > range))) {
                server.Log.Debug(GetString("Rangecheck failed for {0} ({1}, {2}) (rg: {3}/{5}, {4}/{5})", Name, x, y, rgX, rgY, range));
                return false;
            }
            return true;
        }
        ...
        public bool HasBuildPermission(int x, int y, bool shouldWarnPlayer = true) {
            ...
            if (!GetCurrentSettings().DisableBuild || HasPermission(Permissions.antibuild)) {
                ...
                if (!GetCurrentSettings().SpawnProtection || HasPermission(Permissions.editspawn) || !Utils.IsInSpawn(GetCurrentServer(), x, y)) {
                    failure = BuildPermissionFailPoint.Regions;
                    // If they have build permission in this region, then they're allowed to continue
                    if (TShock.Regions.CanBuild(GetCurrentServer(), x, y, this)) {
                        return true;
                    }
                }
            }
            ...
            if (!GetCurrentSettings().SuppressPermissionFailureNotices) {
                switch (failure) {
        ...
        public bool GodMode {
            get {
                var server = GetCurrentServer();
                 return server.CreativePowerManager.GetPower<CreativePowers.GodmodePower>().IsEnabledForPlayer(Index);
            }
            set {
                var server = GetCurrentServer();
                server.CreativePowerManager.GetPower<CreativePowers.GodmodePower>().SetEnabledState(server, Index, value);
            }
        }
                ...
                if (string.IsNullOrEmpty(CacheIP))
                    return
                        CacheIP = RealPlayer ? (Client.Socket.IsConnected()
                                ? Utils.GetRealIP(Client.Socket.GetRemoteAddress().ToString()!)
                                : "")
                            : "127.0.0.1";
                else
                    return CacheIP;
            }
            internal set { CacheIP = value; }
        }
        ...
        public bool SaveServerCharacter() {
            ...
            try {
                if (HasPermission(Permissions.bypassssc)) {
                    server.Log.Info(GetString($"Skipping SSC save (due to tshock.ignore.ssc) for {Account.Name}"));
                    return true;
                }
        ...
        /// </summary>
        public float X {
            get { return RealPlayer ? TPlayer.position.X : FakePlayer.position.X * 16; }
        }
        ...
        public float UnmountedX {
            get { return RealPlayer ? TPlayer.Center.X - 10 : FakePlayer.position.X * 16; }
        }
        ...
        public float CenterX {
            get { return RealPlayer ? TPlayer.Center.X : FakePlayer.position.X * 16; }
        }
        ...
        /// </summary>
        public float RightX {
            get { return RealPlayer ? TPlayer.Right.X : FakePlayer.position.X * 16; }
        }
        ...
        /// </summary>
        public float Y {
            get { return RealPlayer ? TPlayer.position.Y : FakePlayer.position.X * 16; }
        }
        ...
        public float UnmountedY {
            get { return RealPlayer ? TPlayer.Bottom.Y - 42 : FakePlayer.position.Y * 16; }
        }
        ...
        public float CenterY {
            get { return RealPlayer ? TPlayer.Center.Y : FakePlayer.position.Y * 16; }
        }
        ...
        public float UnmountedCenterY {
            get { return RealPlayer ? TPlayer.Bottom.Y - 24 : FakePlayer.position.Y * 16; }
        }
        ...
        public float BottomY {
            get { return RealPlayer ? TPlayer.Bottom.Y : FakePlayer.position.Y * 16; }
        }
        ...
        public void Logout() {
            ...
            Group = TShock.Groups.GetGroupByName(GetCurrentSettings().DefaultGuestGroupName);
            tempGroup = null;
        ...
        public bool Teleport(float x, float y, byte style = 1) {
            ...
            x = Math.Clamp(x,
                640,
                server.Main.rightWorld - 640 - TPlayer.width);

            y = Math.Clamp(y,
                640,
                server.Main.bottomWorld - 640 - TPlayer.height);

            SendTileSquareCentered((int)(x / 16), (int)(y / 16), 15);
            server.RemoteClient.CheckSection(Index, new Vector2(x, y));
            TPlayer.Teleport(server, new Vector2(x, y), style);
            server.NetMessage.SendData((int)PacketTypes.Teleport, -1, -1, NetworkText.Empty, 0, TPlayer.whoAmI, x, y, style);
            return true;
        }
        ...
        /// <summary>
        /// Teleports the player to the world spawn point, or the respective team-based spawnpoint if the world uses them.
        /// </summary>
        ...
        public bool TeleportToWorldSpawn(bool ignoreTeamBasedSpawns = false) {
            var server = GetCurrentServer();
            Point p = new Point(server.Main.spawnTileX, server.Main.spawnTileY);
            if (!ignoreTeamBasedSpawns &&
                server.Main.teamBasedSpawnsSeed &&
                server.ExtraSpawnPointManager.TryGetExtraSpawnPointForTeam(Team, out var spawnPoint)) {
                p = spawnPoint;
            }
        ...
        public void Heal(int health = 600) {
            GetCurrentServer().NetMessage.SendData((int)PacketTypes.PlayerHealOther, -1, -1, NetworkText.Empty, this.TPlayer.whoAmI, health);
        }
        ...
        public virtual bool SendTileRect(short x, short y, byte width = 10, byte length = 10, TileChangeType changeType = TileChangeType.None) {
            var server = GetCurrentServer();
            try {
                server.NetMessage.SendTileSquare(Index, x, y, width, length, changeType);
                return true;
            }
            catch (Exception ex) {
        ...
        public void UpdateSection(Rectangle? rectangle = null, bool isLoaded = false) {
            ...
            if (rectangle.HasValue) {
                for (int i = Netplay.GetSectionX(rectangle.Value.X); i < Netplay.GetSectionX(rectangle.Value.X + rectangle.Value.Width) && i < server.Main.maxSectionsX; i++) {
                    for (int j = Netplay.GetSectionY(rectangle.Value.Y); j < Netplay.GetSectionY(rectangle.Value.Y + rectangle.Value.Height) && j < server.Main.maxSectionsY; j++) {
                        Client.TileSections[i, j] = isLoaded;
                    }
                }
            }
            else {
                for (int i = 0; i < server.Main.maxSectionsX; i++) {
                    for (int j = 0; j < server.Main.maxSectionsY; j++) {
                        Client.TileSections[i, j] = isLoaded;
                    }
                }
            }
        ...
        public virtual void GiveItem(int type, int stack, int prefix = 0) {
            if (GetCurrentSettings().GiveItemsDirectly)
        ...
        private void GiveItemDirectly(int type, int stack, int prefix) {
            ...
            if (ItemID.Sets.IsAPickup[type] || !server.Main.ServerSideCharacter || this.IsDisabledForSSC) {
                GiveItemByDrop(type, stack, prefix);
                return;
            }
        ...
        private void SendItemSlotPacketFor(int slot) {
            ...
            server.NetMessage.SendData(5, this.Index, -1, null, this.Index, slot, prefix, 0f, 0, 0, 0);
        }
        ...
        public virtual void SendMessage(string msg, byte red, byte green, byte blue) {
            ...
            if (this.Index == -1) //-1 is our broadcast index - this implies we're using TSPlayer.All.SendMessage and broadcasting to all clients
            {
                ...
                server.ChatHelper.SendChatMessageToClient(NetworkText.FromLiteral(msg), new Color(red, green, blue), this.Index);
            }
        }
        ...
        public virtual void SendMessageFromPlayer(string msg, byte red, byte green, byte blue, int ply) {
            ...
            server.ChatHelper.BroadcastChatMessageAs((byte)ply, NetworkText.FromLiteral(msg), new Color(red, green, blue));
        }
        ...
        public void SendFileTextAsMessage(string file) {
            ...
            using (var tr = new StreamReader(file)) {
                ...
                while ((foo = tr.ReadLine()) != null) {
                    ...
                    foo = foo.Replace("%map%", (settings.UseServerName ? settings.ServerName : server.Main.worldName));
                    foo = foo.Replace("%players%", String.Join(", ", players));
                    foo = foo.Replace("%specifier%", settings.CommandSpecifier);
                    foo = foo.Replace("%onlineplayers%", Utils.GetActivePlayerCount().ToString());
                    foo = foo.Replace("%serverslots%", settings.MaxSlots.ToString());
        ...
        public virtual void DamagePlayer(int damage, PlayerDeathReason reason) {
            var server = GetCurrentServer();
            server.NetMessage.SendPlayerHurt(Index, reason, damage, (new Random()).Next(-1, 1), false, false, 0, -1, -1);
        }
        ...
        public virtual void KillPlayer(PlayerDeathReason reason) {
            var server = GetCurrentServer();
            server.NetMessage.SendPlayerDeath(Index, reason, 99999, (new Random()).Next(-1, 1), false, -1, -1);
        }
        ...
        public virtual void SetTeam(int team) {
            ...
            TPlayer.team = team;
            server.NetMessage.SendData((int)PacketTypes.PlayerTeam, -1, -1, NetworkText.Empty, Index);
        }
        ...
        public virtual void SetPvP(bool mode, bool withMsg = false) {
            var server = GetCurrentServer();
            TPlayer.hostile = mode;
            server.NetMessage.SendData((int)PacketTypes.TogglePvp, -1, -1, NetworkText.Empty, Index);
            if (withMsg)
                server.ChatHelper.BroadcastChatMessage(NetworkText.FromKey(mode ? "LegacyMultiplayer.11" : "LegacyMultiplayer.12", Name), Main.teamColor[Team]);
        }
                ...
                if ((DateTime.UtcNow - LastDisableNotification).TotalMilliseconds > 5000) {
                    if (flags.HasFlag(DisableFlags.WriteToConsole)) {
                        ...
                        if (flags.HasFlag(DisableFlags.WriteToLog)) {
                            server.Log.Info(GetString("Player {0} has been disabled for {1}.", Name, reason));
                        }
                        else {
                            server.GetExtension<TSServerPlayer>().SendInfoMessage(GetString("Player {0} has been disabled for {1}.", Name, reason));
                        }
                    }
        ...
        public bool Kick(string reason, bool force = false, bool silent = false, string? adminUserName = null, bool saveSSI = false) {
            ...
            if (force || !HasPermission(Permissions.immunetokick)) {
                ...
                server.Log.Info(GetString("Kicked {0} for : '{1}'", Name, reason));
                if (!silent) {
                    if (string.IsNullOrWhiteSpace(adminUserName))
                        Utils.Broadcast(server, GetString("{0} was kicked for '{1}'", Name, reason), Color.Green);
                    else
                        Utils.Broadcast(server, GetString("{0} kicked {1} for '{2}'", adminUserName, Name, reason), Color.Green);
                }
                return true;
            }
            return false;
        ...
        public bool Ban(string reason, string? adminUserName = null) {
            ...
            var serverPlr = GetCurrentServer().GetExtension<TSServerPlayer>();

            if (string.IsNullOrWhiteSpace(adminUserName))
                serverPlr.BCInfoMessage(GetString("{0} was banned for '{1}'.", Name, reason));
            else
                serverPlr.BCInfoMessage(GetString("{0} banned {1} for '{2}'.", adminUserName, Name, reason));
            return true;
        }
        ...
        public virtual void SendData(PacketTypes msgType, string text = "", int number = 0, float number2 = 0f,
            ...
            server.NetMessage.SendData((int)msgType, Index, -1, text == null ? null : NetworkText.FromLiteral(text), number, number2, number3, number4, number5);
        }
        ...
        public virtual void SendDataFromPlayer(PacketTypes msgType, int ply, string text = "", float number2 = 0f,
            ...
            server.NetMessage.SendData((int)msgType, Index, -1, NetworkText.FromFormattable(text), ply, number2, number3, number4, number5);
        }
        ...
        public virtual void SendRawData(byte[] data) {
            ...
            Client.Socket.AsyncSend(data, 0, data.Length, (state) => {
                Client.ServerWriteCallBack(GetCurrentServer(), state);
            });
        }
